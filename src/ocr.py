...
def extract_text(
    ...
    # Objective-Cオブジェクトの参照を保持（finally で解放）
    image_url = None
    ci_image = None
    request = None
    handler = None
    ...
    try:
        ...
        # Objective-Cオブジェクトの参照を明示的に解放
        # pyobjcはPythonのGCに依存するため、参照を削除してGC対象にする
        # 注意: gc.collect()は使用しない（pyobjcのautoreleasepoolと干渉するため）
        del results
        del handler
        del request
        del ci_image
