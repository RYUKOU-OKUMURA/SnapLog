...
| 項目 | 内容 |
|------|------|
| キャプチャ対象 | メインディスプレイ全体（MVP） / アクティブウィンドウ（拡張） |
| 取得情報 | スクリーンショット画像、アクティブウィンドウ名、アプリ名 |
| 一時保存先 | `/tmp/`（処理後に即削除） |

#### 補足（プライバシーとキャプチャ範囲）

- MVPでは実装容易性のため `fullscreen` を前提にするが、背面ウィンドウの情報が混入し得る。
- そのため、将来拡張として `active_window`（前面ウィンドウのみ）を追加し、必要に応じて切り替え可能にする。
...
| 入力 | 当日のJSONLファイル |
| 前処理 | アプリ/ウィンドウ/時間でグルーピングし、LLMの入力上限を超えないよう分割・圧縮してから生成する |
| 出力 | Markdown形式の日報 |
...
| Python | 3.10以上 |
| 必須ライブラリ | pyobjc-framework-Vision / pyobjc-framework-Quartz / PyYAML（Phase 2は requests 追加） |
...
| 項目 | 方針 |
|------|------|
| 保持期間 | デフォルト14日（設定で変更可能） |
| 削除対象 | `logs/` と `reports/` の古い日付ファイル |
| 削除タイミング | 起動時（または1日1回）にクリーンアップ |
